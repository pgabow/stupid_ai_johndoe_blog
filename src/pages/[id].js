    ...
    <>
      ...
      <div className="mx-auto my-5 max-w-3xl content-center px-3">
        <article className="container">
          <h1 className="edit text-secondary">
            <Text text={page.properties.title.title} />
          </h1>
          <p className="mt-2 pb-5 font-montserrat text-lg text-text">
            <small className="text-s mr-4 italic text-cyan-300">{date}</small>{" "}
            <span className="mr-2 inline-block text-yellow-300">
              <AiFillAlert />
            </span>
            <span className="underline">
              {page.properties.autor?.select.name}
            </span>
          </p>

          <section className="mt-30 font-merriweather text-base font-normal">
            {blocks.map((block) => (
              <Fragment key={block.id}>{renderBlock(block)}</Fragment>
            ))}
          </section>
